    ...
    <form action="{{url('admin/config/'.$field->conf_id)}}" method="post">
        ...
        <table class="add_tab">
            ...
            <tr>
                <th>排序：</th>
                <td>
                    <input type="text" class="sm" name="conf_order" value="{{$field->conf_order}}"><span><i
                                class="fa fa-exclamation-circle yellow"></i>默认排序为0</span>
                </td>
            </tr>
            <tr>
                <th>类型：</th>
                <td>
                    <input type="radio" class="sm" name="field_type"
                           @if($field->field_type == 'input') checked @endif
                           value="input" onclick="showTr()">input　
                    <input type="radio" class="sm" name="field_type" value="textarea"
                           @if($field->field_type == 'textarea') checked @endif    onclick="showTr()">textarea　
                    <input type="radio" class="sm"
                           @if($field->field_type == 'radio') checked @endif  name="field_type" value="radio"
            ...
            <tr>
                <th>类型值：</th>
                <td>
                    <input type="text" class="lg field_value" name="field_value" value="{{$field->field_value}}">
